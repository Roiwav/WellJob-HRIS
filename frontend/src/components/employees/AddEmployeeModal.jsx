...
export default function AddEmployeeModal({
  ...
  const complianceReviewWarning = useMemo(() => {
    if (formData.status === "Deployed" && selectedDocuments.length < MIN_DEPLOYED_DOCUMENTS) {
  ...
  const handleChange = (e) => {
    ...
    setErrors((prev) => ({
      ...prev, [name]: "", duplicateConfirm: name === "name" ? "" : prev.duplicateConfirm,
    }));
  };
  ...
  const handleSelectCompany = (company) => {
    ...
    setFilteredCompanies(
      COMPANY_OPTIONS.filter((item) => item.toLowerCase().includes(company.toLowerCase()))
    );
    setShowSuggestions(false);
  };
  ...
  const handleDocumentCheck = (docName) => {
    ...
    setErrors((prev) => ({ ...prev, documents: { ...prev.documents, [docName]: "" } }));
  };
  ...
  const handleFileInput = (docName, file) => {
    ...
    setErrors((prev) => ({ ...prev, documents: { ...prev.documents, [`${docName}_file`]: "" } }));
  };
  ...
  const validateForm = () => {
    ...
    if (!trimmedName) {
      ...
      nextErrors.name = "Full name must contain letters only. Numbers and special symbols are not allowed.";
            ...
            <div className="flex h-full flex-col">
              ...
              <div className="mt-8 rounded-2xl bg-white/10 p-4 ring-1 ring-white/15">
                ...
                {remainingDocuments > 0 && (
                  <p className="mt-3 text-xs text-white/70">
                    {remainingDocuments} compliance document{remainingDocuments === 1 ? "" : "s"} remaining to complete.
                  </p>
                )}
              </div>
              ...
              <div className="mt-5 space-y-3 text-sm">
                ...
                <div className="rounded-2xl bg-white/10 p-4 ring-1 ring-white/15">
                  ...
                  <p className="mt-1 text-white/75">{completedDocuments.length}/{DOCUMENT_OPTIONS.length} completed</p>
                </div>
              </div>
            </div>
          </aside>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {duplicatePreview && (
                    <StatusPill tone={duplicateConfirmed ? "amber" : "red"}>
                      <FiAlertTriangle /> {duplicateConfirmed ? "Duplicate Verified" : "Possible Duplicate"}
                    </StatusPill>
                  )}
                </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-6">
                  ...
                  <main className="space-y-6">
                    <section className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                      <div className="mb-5 flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-extrabold text-gray-900 dark:text-white">Basic Employee Information</h3>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Employee ID remains the official unique identifier.</p>
                        </div>
                      </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                          <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Employment Status</label>
                          <div className="relative">
                            <select name="status" value={formData.status} onChange={handleChange} className="w-full appearance-none rounded-2xl border border-gray-200 bg-white px-4 py-3 pr-10 text-sm font-semibold text-gray-900 outline-none transition focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:border-white/10 dark:bg-slate-800 dark:text-white">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <input type="text" name="name" placeholder="e.g. Juan D. Dela Cruz" value={formData.name} onChange={handleChange} onBlur={handleNameBlur} autoComplete="off" className={`w-full rounded-2xl border bg-white px-4 py-3 text-sm font-semibold text-gray-900 outline-none transition focus:ring-4 dark:bg-slate-800 dark:text-white ${errors.name || duplicatePreview ? "border-red-500 focus:ring-red-500/10" : "border-gray-200 focus:border-indigo-500 focus:ring-indigo-500/10 dark:border-white/10"}`} />
                          <ErrorText>{errors.name}</ErrorText>
                        </div>

                        {formData.status === "Deployed" && (
                          <div className="md:col-span-2">
                            <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Company Assignment</label>
                            <div className="relative">
                              <input type="text" name="company" value={formData.company} onChange={handleChange} onFocus={() => { setFilteredCompanies(COMPANY_OPTIONS.filter((company) => company.toLowerCase().includes(formData.company.toLowerCase()))); setShowSuggestions(true); }} onBlur={() => setTimeout(() => setShowSuggestions(false), 150)} placeholder="Type or select company name..." autoComplete="off" className={`w-full rounded-2xl border bg-white px-4 py-3 text-sm font-semibold text-gray-900 outline-none transition focus:ring-4 dark:bg-slate-800 dark:text-white ${errors.company ? "border-red-500 focus:ring-red-500/10" : "border-gray-200 focus:border-indigo-500 focus:ring-indigo-500/10 dark:border-white/10"}`} />
                              {showSuggestions && filteredCompanies.length > 0 && (
                                <div className="absolute z-50 mt-2 max-h-56 w-full overflow-y-auto rounded-2xl border border-gray-200 bg-white p-2 shadow-2xl dark:border-white/10 dark:bg-slate-900">
                                  {filteredCompanies.map((company) => (
                                    <button key={company} type="button" onMouseDown={(e) => e.preventDefault()} onClick={() => handleSelectCompany(company)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-gray-800 transition hover:bg-indigo-50 dark:text-white dark:hover:bg-white/10">
                                      <FiBriefcase className="text-indigo-500" /> {company}
                                    </button>
                                  ))}
                                </div>
                              )}
                        ...
                        {duplicatePreview && (
                          <div className="md:col-span-2 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                            <div className="flex gap-3">
                              ...
                              <div>
                                <p className="font-extrabold">Possible duplicate employee found.</p>
                                <p className="mt-1">Existing record: <b>{duplicatePreview.name}</b> ({duplicatePreview.id}). Verify through resume or supporting documents before saving.</p>
                                <label className="mt-3 flex cursor-pointer items-start gap-2 text-xs font-bold">
                                  <input type="checkbox" checked={duplicateConfirmed} onChange={(e) => { setDuplicateConfirmed(e.target.checked); setErrors((prev) => ({ ...prev, duplicateConfirm: "" })); }} className="mt-0.5" />
                                  I verified the resume/supporting documents and confirm this is a different employee.
                                </label>
                  ...
                  <aside className="space-y-4">
                    <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                      <div className="mb-4 flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-extrabold text-gray-900 dark:text-white">Record Summary</h3>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Live preview before review.</p>
                        </div>
                      </div>
                      <div className="space-y-3 text-sm">
                        ...
                        <SummaryRow label="Documents" value={`${completedDocuments.length}/${DOCUMENT_OPTIONS.length} completed`} />
                      </div>
                    </div>
                    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                      ...
                      <p className="leading-5">Verify duplicate names using supporting documents.</p>
                    </div>
                  </aside>
                </div>

                <section className="rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/60">
                  <button type="button" onClick={() => setShowDocuments((prev) => !prev)} className="flex w-full items-center justify-between gap-4 p-5 text-left">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-extrabold text-gray-900 dark:text-white">Compliance Documents</h3>
                        <p className="text-xs text-gray-500 dark:text-gray-400">Click to show or hide document requirements and proof uploads.</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <StatusPill tone="slate">{completedDocuments.length}/{DOCUMENT_OPTIONS.length} completed</StatusPill>
                      {errors.documents.general && (
                        <p className="mt-2 text-xs font-semibold text-red-600 dark:text-red-400">{errors.documents.general}</p>
                      )}
                      <FiChevronDown className={`text-gray-400 transition-transform ${showDocuments ? "rotate-180" : ""}`} />
                    </div>
                  </button>
                    ...
                    <div className="border-t border-gray-200 p-5 dark:border-white/10">
                      <div className="grid min-w-0 gap-4 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
                        {formData.documents.map((doc) => {
                          ...
                          const isRisky = doc.checked && (status === "Expired" || status === "Expiring Soon");

                          return (
                            <div key={doc.name} className={`rounded-2xl border p-4 transition ${doc.checked ? "border-indigo-200 bg-indigo-50/60 dark:border-indigo-500/30 dark:bg-indigo-500/10" : "border-gray-200 bg-white hover:border-indigo-200 dark:border-white/10 dark:bg-slate-900/50 dark:hover:border-indigo-500/30"}`}>
                              <label className="flex cursor-pointer items-start gap-3">
                                <input type="checkbox" checked={doc.checked} onChange={() => handleDocumentCheck(doc.name)} className="mt-1 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center justify-between gap-3">
                                    <p className="font-bold text-gray-900 dark:text-white">{doc.name}</p>
                                    {doc.expirable ? (<StatusPill tone="amber">Expirable</StatusPill>) : (<StatusPill tone="green">Permanent</StatusPill>)}
                                  </div>
                                  ...
                                  {doc.checked && (
                                    <div className="mt-4 space-y-3">
                                      {doc.expirable && (
                                        <div>
                                          <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">Expiration Date</label>
                                          <input type="date" value={doc.expirationDate} onChange={(e) => handleDocumentDateChange(doc.name, e.target.value)} className={`w-full rounded-xl border bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:ring-4 dark:bg-slate-800 dark:text-white ${errors.documents[doc.name] ? "border-red-500 focus:ring-red-500/10" : "border-gray-200 focus:border-indigo-500 focus:ring-indigo-500/10 dark:border-white/10"}`} />
                                          <ErrorText>{errors.documents[doc.name]}</ErrorText>
                                          {doc.expirationDate && (
                                            <p className={`mt-1.5 text-xs font-semibold ${isRisky ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}>
                                              Status: {status}
                                            </p>
                                          )}
                                        </div>
                                      ...
                                      <div>
                                        <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-500">Proof Upload</label>
              ...
              <footer className="mt-6 flex flex-col-reverse gap-3 border-t border-gray-200 pt-5 dark:border-white/10 sm:flex-row sm:justify-end">
                <button type="button" onClick={onClose} className="rounded-2xl border border-gray-200 px-5 py-3 text-sm font-bold text-gray-700 transition hover:bg-gray-50 dark:border-white/10 dark:text-gray-300 dark:hover:bg-white/10">
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-white/10 bg-white p-6 shadow-2xl dark:bg-slate-900">
            ...
            {duplicatePreview && !duplicateConfirmed && (
              <div className="mb-4 rounded-2xl border border-red-300 bg-red-50 p-4 dark:border-red-500/30 dark:bg-red-500/10">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-bold text-red-700 dark:text-red-300">Duplicate Not Verified</p>
                    <p className="text-sm text-red-700 dark:text-red-200">Please confirm duplicate verification before saving.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mt-5 rounded-2xl border border-gray-200 p-5 dark:border-white/10">
              <p className="mb-4 text-sm font-extrabold text-gray-900 dark:text-white">Compliance Documents</p>
              {selectedDocuments.length > 0 ? (
                <div className="space-y-3">
                  {selectedDocuments.map((doc) => {
                    const masterDoc = DOCUMENT_OPTIONS.find((item) => item.name === doc.name);
                    const isExpirable = masterDoc?.expirable;
                    const status = isExpirable ? getDocumentStatus(doc.expirationDate) : "Permanent";
                    const isValid = !isExpirable || (status !== "Expired" && status !== "Expiring Soon");

                    return (
                      <div key={doc.name} className="flex flex-col justify-between gap-3 rounded-2xl bg-gray-50 px-4 py-3 dark:bg-slate-800 md:flex-row md:items-center">
                        <div>
                          <p className="font-bold text-gray-900 dark:text-white">{doc.name}</p>
                          {doc.file ? (
                            doc.file.type?.includes("image") ? (
                              <img src={doc.file.url || URL.createObjectURL(doc.file)} alt={doc.file.name} className="mt-2 max-h-40 rounded border" />
                            ) : doc.file.type === "application/pdf" ? (
                              <iframe src={doc.file.url || URL.createObjectURL(doc.file)} title={doc.file.name} className="mt-2 w-full h-40 rounded border" />
                            ) : (
                        ...
                        <div className="text-sm font-semibold text-gray-600 dark:text-gray-300">
                          {isExpirable ? (<>Expires: {doc.expirationDate || "-"} • {status}</>) : ("Permanent Document")}
                        </div>
                        {isValid && (<span className="text-xs text-green-600 dark:text-green-400 font-bold">✔ Document is valid</span>)}
                      </div>
                    );
                  })}
                </div>
              ) : (
                <p className="text-sm text-gray-500 dark:text-gray-400">No compliance documents selected.</p>
              )}
            </div>
