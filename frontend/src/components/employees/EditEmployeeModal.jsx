...
function buildInitialFormData(employeeToEdit) {
  const parsedDocs = parseEmployeeDocuments(employeeToEdit?.documents);

  return {
    name:
      employeeToEdit?.name ||
      employeeToEdit?.fullName ||
      employeeToEdit?.fullname ||
      "",
...
export default function EditEmployeeModal(props) {
  const initialFormData = useMemo(
    () => buildInitialFormData(props.employeeToEdit),
    [props.employeeToEdit]
  );

  return (
    <EditEmployeeModalContent
      key={props.employeeToEdit?.id || "employee-edit-modal"}
      {...props}
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div>
                          ...
                          <input
                            ...
                            disabled
                            className="w-full rounded-2xl border border-gray-200 bg-gray-100 px-4 py-3 text-sm font-bold text-gray-500 outline-none dark:border-white/10 dark:bg-slate-800 dark:text-gray-400"
                          />
                        </div>
