  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-lg dark:border-slate-700 dark:bg-slate-800">
      ...
      {payload.map((entry, index) => (
        ...
        >
          Utilization: {Number(entry.value || 0)}%
        </p>
      ))}
    </div>
  );
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">
          Tracks active workforce deployment utilization across reporting
          periods.
        </p>
      </div>
      ...
      {chartData.length === 0 ? (
        ...
        <div
          ...
          role="img"
          aria-label="Line chart showing deployment utilization trends"
        >
